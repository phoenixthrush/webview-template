...
ifeq ($(OS),Windows_NT)
	...
	RUN = /C "set __COMPAT_LAYER=RUNASINVOKER && start /wait ./build/bin/$(BINARY)"
else
	CREATE_BUILD_DIRECTORY = mkdir -p build
	DELETE_BUILD_DIRECTORY = rm -rf build

	CHECK_DEPENDENCIES = bash src/helpers/deps.sh
	PACK_USING_UPX = bash src/helpers/pack.sh

	RUN = ./build/bin/$(BINARY)
endif
...
prepare:
	...
	python src/helpers/preprocess.py -i site/index.html -o build/index.modified.html
